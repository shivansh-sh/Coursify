    ...
    return <Card variant="outlined" style = {{
       ...
       <Typography textAlign={"center"}><p style={{margin:1}}>{course.description}</p></Typography> 
        ...
        <div style={{ display: "flex", justifyContent: "space-around"}}>
            <Typography><button style={{color: "white", background: "brown", padding: 5, fontSize: 13, outline: "auto"}}
            onClick={()=> {navigate("/course/"+course._id)}}
            >Update</button></Typography>
            <Typography><button style={{color: "white", background: "green" , padding: 5, fontSize: 13, outline: "auto"}}
            onClick={() => {
                fetch("https://localhost:4000/admin/courses/"+course._id, {
                    ...
                    res.json().then((data) => {
                        props.setCourses((prevCourses) => prevCourses.filter((c) => c._id !==course._id))
                        alert(data.message);
                        navigate("/showcourses", {replace : true})                   
                })
            })
            }}
